...
public class ProjectCreateActivity extends AppCompatActivity implements ColorPickerView.OnColorChangedListener {
    ...
    private void addProject()
    {
        ...
        Rx2Apollo.from(client.mutate(new CreateProject(idWorkspace, projectInput)))
                ...
                .subscribeWith(new DisposableObserver<Response<CreateProject.Data>>() {
                    ...
                    public void onNext(Response<CreateProject.Data> dataResponse) {
                        if (dataResponse.errors().isEmpty()) {
                            projectController.createProject(dataResponse.data().createProject(),
                                    name, description, color, workspace.getId());
